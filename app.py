...
# V30: keep OCR deliberately light enough for a small/free server.
MAX_IMAGE_SIDE = 2200
PDF_SCALE = 1.7
...
def read_img(im):
    p = prep(im)
    # One Tesseract pass instead of image_to_data + image_to_string.
    data = pytesseract.image_to_data(
        p, lang="fra+eng", config="--oem 3 --psm 6",
    ...
    lines = [" ".join(v) for _, v in sorted(groups.items())]
    txt = "\n".join(lines)
...
def pdf_images(raw):
    ...
    try:
        for pg in doc:
            pix = pg.get_pixmap(matrix=fitz.Matrix(PDF_SCALE, PDF_SCALE), alpha=False)
            yield Image.open(io.BytesIO(pix.tobytes("jpeg", jpg_quality=82))).copy()
    finally:
        doc.close()
...
def money(x):
    return clean(x).replace("€", "").replace("\xa0", " ")
...
def extract(text):
    ...
    o["valid_until"] = grab(r"(?:valable jusqu(?:'|’)?au|valable jusqu(?:'|’)?au|valable)\D{0,15}(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})", text)
    ...
    for raw in text.splitlines():
        ...
        if len(line) < 5 or not re.search(r"\d", line):
            continue
        if re.search(r"\b(total|TVA|net à payer|sous total)\b", line, re.I):
            continue
...
@app.get("/")
def home():
    return send_from_directory("static", "Auto_Clair_V30.html")
...
def ocr():
    ...
    if not files:
        return jsonify(error="Aucun document reçu"), 400
    texts, confs, pages = [], [], 0
    try:
        for f in files:
            raw = f.read()
            ...
            if f.filename.lower().endswith(".pdf") or f.mimetype == "application/pdf":
                for im in pdf_images(raw):
                    t, c = read_img(im)
                    texts.append(t); confs.append(c); pages += 1
                    im.close()
            else:
        ...
        result.update(
            ...
            note="OCR côté serveur optimisé pour limiter la mémoire. Vérifiez les champs avant de lancer l’analyse."
        )
        return jsonify(result)
    except Exception as e:
        app.logger.exception("OCR failure")
...
def unhandled(e):
    ...
    return jsonify(error="Erreur interne pendant le traitement du dossier."), 500
